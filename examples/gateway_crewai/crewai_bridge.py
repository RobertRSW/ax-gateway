...
a reply through the Gateway end to end. Real CrewAI orchestration with
multi-agent crews, role-based Tasks, tool calls mapped to Gateway tool
bubbles, and streaming events is a follow-up.

If `crewai` is importable, the bridge logs that and still returns the
stub reply. A real Crew kickoff requires an LLM endpoint + credentials
and is intentionally out of scope for this slice. If `crewai` is not
...
def _run_stub_crew(prompt: str) -> str:
    """Return a stub reply, optionally noting whether crewai is installed.
    ...
    iterations will instantiate a real CrewAI Crew with role-based
    Agents and Tasks, map per-task tool calls to Gateway tool bubbles,
    and stream the crew's per-task progress as activity events.
    """
    try:
    ...
    emit_event(
        {
            "kind": "activity",
            "activity": "crewai module loaded; stub reply only (real Crew execution is a follow-up)",
        }
    )
