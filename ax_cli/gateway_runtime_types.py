...
def agent_template_catalog() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        "crewai": {
            ...
            "operator_summary": "Gateway-managed CrewAI bridge. Initial cut ships with a stub reply; real Crew execution is a follow-up.",
            "recommended_test_message": "Reply with: CrewAI round trip OK.",
            "what_you_need": [
                "Python 3.11+ on this machine (the bridge runs as a Gateway-managed subprocess).",
                "For real Crew execution, install crewai plus an LLM endpoint and credentials. The stub bridge runs without it.",
            ],
            "setup_skill": "gateway-agent-setup",
            "setup_skill_path": str(skill_path),
            "defaults": {
