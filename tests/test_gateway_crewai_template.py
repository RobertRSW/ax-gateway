...
def test_crewai_template_is_registered() -> None:
    ...
    assert "crewai" in catalog, (
        "crewai template missing from agent_template_catalog. "
        "Should sit alongside langgraph / strands / ollama / hermes."
    )

    template = agent_template_definition("crewai")
    assert template["id"] == "crewai"
    assert template["runtime_type"] == "exec", (
        "Reuses the exec runtime adapter (same precedent as langgraph / strands / ollama). "
